<template>
  <div class="hello">
    ...
    <b-container>
        ...
        <div class="rIcon">
          <b-icon-arrow-right v-on:click="nextPage" style="color: lightGrey; width: 120px; height: 120px"></b-icon-arrow-right>
        </div>
        <div class="lIcon" id="leftArr">
          <b-icon-arrow-left v-on:click="prevPage" style="color: lightGrey; width: 120px; height: 120px"></b-icon-arrow-left>
        </div>
      ...
      <b-row class="cardRow">
        ...
        <b-col cols="4" v-bind:key="index" v-for="(value, index) in characterData"   >
          <div class="text"  >
            <b-card v-on:click="handleClick(value)" id="no" no-body className="card" class="overflow-hidden">
              <b-row no-gutters>
...
export default {
  ...
  created () {
    for (let index = this.startIndex; index < this.endIndex; index++) {
      ...
      axios.get(`http://swapi.dev/api/people/${index}/`)
    .then(response => {
